...
*           Sync    response or comm error beginning with "FAILURE
*/
function xrxScanV2InitiateScanJob( url, scanV2JobTicket, callback_success, callback_failure, timeout, async )
{
    if((url == null) || (url == ""))
        url = "http://127.0.0.1";
	var sendUrl = url + XRX_SCANV2_PATH;
...
/**
* This function gets the image from the device
*
* @param	url					destination address
* @param	jobId               job id of parsed from return from scan request
* @param    token               token in string form
* @param	callback_success	function to callback upon successfull completion
* @param	callback_failure	function to callback upon failed completion
* @param	timeout				function to call an error routine after a set amount 
...
*           Sync    response or comm error beginning with "FAILURE
*/
function xrxScanV2GetImage( url, jobId, token, callback_success, callback_failure, timeout, async )
{
    if((url == null) || (url == ""))
        url = "http://127.0.0.1";
	var sendUrl = url + XRX_SCANV2_PATH;
	var sendReq = xrxScanV2GetImageRequest( jobId, token );
	return xrxCallWebservice( sendUrl, sendReq, callback_success, callback_failure, timeout, null, null, null, async );
}   
...
* @return	string	        xml request
*/
function xrxScanV2GetImageRequest( jobId, token )
{
	return	XRX_SOAPSTART
			+ xrxCreateTag( 'GetImageFileRequest', XRX_SCANV2_NAMESPACE, 
...
/**
* This function pulls the queue data from the response.
*
* @param	response	webservice response in string form
* @return	string		job data
*/
function findMtomData( response )
{
    var index = response.indexOf( "?xml version" );
    index = response.indexOf( "<?xml version", index + 1 );
    return response.substring( index, response.lastIndexOf( ">" ) + 1 );
}
